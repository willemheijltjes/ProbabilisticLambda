...
\begin{abstract}
A notion of probabilistic lambda-calculus usually comes with a prescribed reduction strategy, typically call-by-name or call-by-value, as the calculus is non-confluent and these strategies yield different results. This is a break with one of the main advantages of lambda-calculus: confluence, which means that results are independent from the choice of strategy.
...
\section{Projective Head Reduction and Call-By-Name}
\label{sec:projective-head-reduction}
...
Observe that while projective head reduction cannot reduce under a sum $\+$, permutative reduction \emph{can}, so that a $\rw_\ph$ step under an external sum $+$ can be simulated by a $\rws_\perm$ reduction under an internal sum $\+$.

Then by applying projective head reduction recursively, we may characterize $\perm$-normal forms directly.
 
\begin{prop}
Permutative normal forms are given by: $(H[x])_\perm=H_\perm[x]$ and
\[
	(H[\,\trm{!a.N}])_\perm =
	\left\{\begin{array}{l@{\qquad}l}
		H_\perm[N_\perm]           & \text{if}~a\notin\fl N
	\\\\H_\perm[\proj a0N_\perm]\+
		H_\perm[\proj a1N_\perm]   & \text{otherwise}
	\end{array}\right.
\]
where the $\perm$-normal form $H_\perm[\,]$ of a head context $H[\,]$ is given by:
\[
	[\,]_\perm = [\,]
\qquad
	(\x.H[\,])_\perm = \x.H_\perm[\,]
\qquad
	(H[\,]\,N)_\perm = H_\perm[\,]\,N_\perm
\]
\end{prop}

\begin{proof}
Observe that the above defines a term in $\perm$-normal form (Proposition~\ref{prop:normal-forms}). Then by confluence of $\rw_\perm$ (Lemma~\ref{lem:confluence-perm}), all we need to exhibit is a $\perm$-reduction from a term $M$ to the above characterization of $M_\perm$. We proceed by induction on the size of $M$. The case $M=H[x]\rws_\perm H_\perm[x]$ is immediate by induction. The case $M=H[\trm{!a.N}]$ follows by a projective head step $\rw_\ph$, which is simulated by permutative reduction by Proposition~\ref{prop:permutative-simulates-projective} above; and then by induction.
\[
\begin{array}{rcl@{\qquad}l@{\qquad}l}
	H[\trm{!a.N}] 
	 & \rws_\perm & H[N]  
	  & \text{if}~a\notin\fl N
       & (\text{Prop.~\ref{prop:permutative-simulates-projective}})
\\   & \rws_\perm & H_\perm[N_\perm] 
      && (\ih)
\\ \\
	H[\trm{!a.N}] 
	 & \rws_\perm & H[\proj a0N]\+H[\proj a1N] 
	  & \text{if}~a\in\fl N
	   & (\text{Prop.~\ref{prop:permutative-simulates-projective}})
\\	 &\rws_\perm & H_\perm[\proj a0N_\perm]\+ H_\perm[\proj a1N_\perm] 
      && (\ih)
\end{array}
\]
\end{proof}

We use the above to give a more formal connection between our Probabilistic Event $\lambda$-Calculus and a call-by-name probabilistic $\lambda$-calculus. Via the encoding
\[
	N\+M \quad\stackrel\Delta=\quad \trm{!a. N +a M}
\]
the following calculus emerges as a fragment of the $\PEL$, using standard $\beta$-reduction together with projective head reduction for probabilistic evaluation.
\[
	N,M \quad\coloneqq\quad x ~\mid~ \x.N ~\mid~ NM ~\mid~ N\+M
\]
\[
	C[\,(\lambda x.N)M\,]~\rw_\beta~C[\,M[N/x]\,]
	\qquad
	H[N\+M]~\rw_\ph~H[N] + H[M]
\]
Here, $\beta$-reduction is unrestricted, but one may obtain a more standard call-by-name probabilistic $\lambda$-calculus by restricting $\beta$-reduction to a head context $H[\,]$, or by restricting both reductions to \emph{applicative} contexts $A[\,]~\coloneqq~[\,]\mid A[\,]\,N$ for a calculus with \emph{weak head reduction}. 

Any term of the $\PEL$ maps into this calculus by $\perm$-normalization, since $\perm$-normal forms are within the call-by-name fragment above. Note however that $\perm$-normalization may induce exponential growth. On a practical level, this gives our calculus greater expressiveness than standard probabilistic calculi.

\section{Projective reduction}
\label{sec:projective-reduction-x}

A gap remains between which generators projective head reduction \emph{does} reduce, and which generators we \emph{should} be able to reduce, because they will not be duplicated. In particular, to interpret call-by-value probabilistic reduction in Section~\ref{sec:cbv}, we would like to reduce under other generators. However, permutative reduction does not permit exchanging generators, and so only simulates reducing in head position. While (independent) probabilistic events are generally considered interchangeable, it is a question whether the below equivalence is desirable.
%
\begin{align}
...
\begin{thm}
If $N\rws_{\val,\beta\val}V$ (\ie\ $N$ reduces to a sum containing the value $V$) then $\uncbv N^s\rws_{\pi,\beta}\uncbv V$ for some stream $s\in\Streams$.
\end{thm}
