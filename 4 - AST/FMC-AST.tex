...
The term $\term{(\heads + (\tails + \,i))^i}$ chooses \emph{heads}, the choice $\heads$, with probability $\frac12$, \emph{tails} $\tails$ with probability $\frac14$, and iterates with probability $\frac14$, to give an overall probability of $\frac 23$ for $\heads$ and $\frac 13$ for $\tails$. We illustrate it as follows; it will remain a running example. %throughout the paper.
\[
	\term{(\heads + (\tails + \,i))^i}
\quad=\quad
\vc{%
