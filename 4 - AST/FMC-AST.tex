...
\[
	...
	\left\{\begin{array}{ll}
		\term{s} & \text{if}~s~\in~F
	\\	{\colorterm\lfloor\D{{\sem{C(t_1)}}.p_1,..,{\sem{C(t_n)}}.p_n}\rfloor^s} & \text{where}~\delta(s)=\D{t_1.p_1,..,t_n.p_n}	
	\end{array}\right.
\]
...
\end{array}
\]
Its interpretation is as follows, writing $\term{M+N+P}$ for the term $\colorterm\lfloor\D{M./13,N./13,P./13}\rfloor$.
\[
\begin{aligned}
