...
\end{array}
\]
\caption{Typing rules for ground terms, with illustrating diagrams. The double arrows represent multiple branches. In the second rule, the types for $\term M$ and $\term N$ divide into overlappings parts $\type{p_J}$ and $\type{`r\!_J}$, and non-overlapping parts $\type{p_I}$ and $\type{`r\!_K}$, where $I$, $J$, and $K$ are pairwise disjoint sets of choices.}
