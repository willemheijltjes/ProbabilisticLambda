...
\begin{tikzpicture}[thick,x=18pt,y=15pt,outer sep=0pt,inner sep=2pt]
	...
	\coordinate (b) at (1.25,-1.6);% \node[gray,circle,draw,inner sep=0pt,minimum size=2pt] at (b) {};
    	...
    	\ifx#10
		\draw[rounded corners=5pt] (m) -- (.5,-1) .. controls (a) and (b) .. (1.25,-.75) --node[pos=0.3,anchor=west]{\,$\termcolor\scriptstyle{j}$} (1.25,1.5) -- (0,1.5) -- (0,1);
    	\fi	
...
\author{Anonymous authors}{No affiliation}{}{}{}
...
\authorrunning{Anonymous authors}%{U. Dal Lago and W. Heijltjes}
...
\begin{abstract}
We present a type system for almost-sure termination (AST) in higher-order probabilistic computation. Our vehicle is the recent Functional Machine Calculus (FMC), which combines the lambda-calculus with effectful imperative computation. Importantly for our purpose, it models branching and looping computation with sum types and a typed iterator construct. Our key observation is that by replacing sum types with probability distributions, types ensure that iteration has a non-zero chance of exiting, which then gives AST in a simple and natural way. 

Concretely, our type system assigns explicit probability distributions to termination outcomes, allowing probabilistic behaviour to be tracked compositionally at the type level. Types then soundly guarantee almost-sure termination, even in the presence of higher-order functions. The fragment of ground terms, which consists purely of the branching and iteration constructs with probabilistic sampling, is equivalent to finite Markov chains and is positively AST (expected evaluation time is finite). The full calculus is more expressive, which we demonstrate through an example term with infinite expected evaluation time.

%We exploit this insight by shifting attention from recursion to typed iteration, allowing probabilistic behaviour to be tracked compositionally at the type level. For this purpose we introduce a novel probabilistic type system that assigns explicit probability distributions to termination outcomes, and we show that these types soundly guarantee almost-sure termination, even in the presence of higher-order functions. We study the expressive power of the introduced system through an embedding of Markov chains, and prove that \emph{positive} almost-sure termination does not hold at higher-order, in general.
\end{abstract}
...
We are interested in applying the toolset of the $\lambda$-calculus, in particular confluent reduction and type systems, to probabilistic computation. We build on \cite{DalLago-Guerrieri-Heijltjes-2020}, which restores confluence to a probabilistic $\lambda$-calculus, the \emph{probabilistic event $\lambda$-calculus} (PEL), by decomposing probabilistic choice into a \emph{sampling} construct and a \emph{conditional}. For this calculus,~\cite{Antonelli-DalLago-Pistone-2022} and~\cite{Heijltjes-Majury-2025} give essentially the same probabilistic type system for the PEL to give lower bounds on the probability of termination.
%In~\cite{DalLago-Guerrieri-Heijltjes-2020} we restored confluence to a probabilistic $\lambda$-calculus, the \emph{probabilistic event $\lambda$-calculus} (PEL), by decomposing probabilistic choice into a \emph{sampling} construct and a \emph{conditional}. Then in~\cite{Antonelli-DalLago-Pistone-2022} and~\cite{Heijltjes-Majury-2025}, from different inspirations we converged on essentially the same probabilistic type system for the PEL to give lower bounds on the probability of termination.
In this paper we further extend the calculus, and develop a type system for \emph{almost-sure termination} (AST): termination with probability one. In doing so, our objective differs from that of many existing works in this area, which aim to capture as many AST programs as possible~\cite{DalLago-Grellois-2019,DalLago-Faggian-Ronchi-2021}, or even to study the feasibility of capturing all such programs~\cite{Kobayashi-DalLago-Grellois-2020}. Instead, we seek to understand the extent to which almost-sure termination can be obtained and explained in a natural manner, possibly using well-established methods, and guaranteed by a minimal extension to simple types. More powerful methods such as dependent types and intersection types may later build on this along standard lines.
...
The type system we introduce in this paper, simplified for this introduction to omit inputs and return values, records the probability of terminating with each given choice. A term $\term M$ is typed as follows, indicating that it may choose $i_k$ with probability $p_k$ from the choices $i_1$ through $i_n$. For simplicity, we restrict ourselves to rational probabilities, $p,q\in[0,1]\cap\mathbb{Q}$.
\[
...
\[
	\term{\heads + (\tails + \,i) : /12\heads * /14\tails * /14i}
\]
\end{example}
...
\begin{example}
The type for our example term is given by taking the type $\type{/12\heads * /14\tails * /14i}$ of the body of the loop, removing the element $\type{/14i}$, and then multiplying the remaining sub-distribution $\type{/12\heads * /14\tails}$ by $\frac 43$ to renormalize it, which yields the following.
\[
...
\subsubsection*{A Typed Term Calculus for Finite Markov Chains}

Further introducing the calculus, we first consider the fragment of \emph{ground terms}, as follows:
\[
	\text{\emph{Gound terms:}}\qquad \term{M,N}~\coloneqq~\term i~\mid~\term{M;i->N}~\mid~\term{M^i}~\mid~\term{?a.a}
\]
The first three are the \emph{control} constructs, introduced in \cite{Heijltjes-2025-MFPS}, which implement branching and iterating computation. The first (a \emph{choice} $\term i$) and the third (a \emph{loop} $\term{M^i}$), we have seen. The second is a \emph{case} $\term{M;i->N}$ which first evaluates $\term M$, and if $\term M$ terminates with $i$ then continues with $\term N$, otherwise discarding it. The control constructs reduce by the following rules:
\[
	\term{i;i->M}~\rw~\term{M} \qquad\quad \term{j;i->M}~\rw~\term j\quad (i\neq j) \qquad\quad \term{M^i}~\rw~\term{M;i->M^i}
\]
These constructions generalise sequential computation, which uses a single choice $\star$, with imperative $\emph{skip}$ as $\term*$ and \emph{sequencing} as $\term{M;N}=\term{M;*->N}$. The choice and case constructs are essentially \emph{throw} and \emph{try/catch} of exception handling, with exceptions $e$ as choices:
\[
	\mathsf{throw}~e~=~\term e \qquad\qquad \mathsf{try}~M~\mathsf{catch}~e~N~=~\term{M;e->N}
\]
Since cases are fall-through, they may be iterated to give a standard case switch. A conditional then encodes as below, where cases associate left, $\term{M; i->N ; j->P}=\term{(M; i->N) ; j-> P}$. A while-loop, below that, is as follows: if $B$ returns $\bot$, then $M$ is skipped, the loop exited, and $\bot$ caught to return $\star$ (success); if $B$ returns $\top$, then $M$ is evaluated, iterating upon $\star$.
\[
\begin{array}{r@{}l}
	\mathsf{if}~B~\mathsf{then}~M~\mathsf{else}~N & ~=~\term{B;\top->M;\bot->N}
\\[5pt]	\mathsf{while}~B~\mathsf{do}~M            & ~=~\term{(B;\top->M)^\star;\bot->*}
\end{array}
\]
The fourth construct $\term{?a.a}$ is the \emph{generator} $\term{?a.M}$ of the probabilistic event $\lambda$-calculus~\cite{DalLago-Guerrieri-Heijltjes-2020} restricted to a variable $\term a$. The generator reduces as below: it flips a coin and binds the result, $\top$ or $\bot$, to the variable $\term a$ in $\term M$.
%
%The fourth construct $\term{?a.a}$ implements a fair coin toss, reducing to $\term\top$ or $\term\bot$ with probability $\frac12$ each. It combines a probabilistic \emph{generator} $\term{?a}$ with a variable $\term a$, where the former samples $\top$ or $\bot$ and substitutes it for $\term a$:
\[
	\term{?a.M}~\rw~\term{\{\top/a\}M}~\oplus~\term{\{\bot/a\}M}
\]
%We introduced the generator in~\cite{DalLago-Guerrieri-Heijltjes-2020} to obtain confluence in a probabilistic $\lambda$-calculus while expressing call--by--value and call--by--name behaviour through the syntax (as opposed to fixing a global strategy). The call--by--name probabilistic sum is then expressed as follows.
The restricted construct $\term{?a.a}$ thus reduces to $\top$ or $\bot$ with equal probability. Using it, a probabilistic sum encodes as follows.
\[
	M\oplus N~=~\term{?a.a;\top->M;\bot->N}
\]

\begin{example}
Our running example term unfolds as a ground term as follows.
\[
	\term{(\heads+(\tails+\,i))^i}~=~\term{`{\big(}\,?a.a ; \top->\heads ; \bot->(\,?b.b ; \top->\tails ; \bot -> i)`{\big)}^i}
\]
\end{example}

\begin{remark}
The generator $\term{?a.M}$ represents a \emph{fair} coin toss, with equal probability for $\top$ and $\bot$. We write $\term{?a_p.M}$ for an unfair coin with probability $p$ for $\top$ and $1-p$ for $\bot$. This may either be considered a primitive construct generalising $\term{?a.M}$, or constructed from fair coin tosses with iteration in the way our running example term generates $\frac23$ and $\frac13$.
\end{remark}

We now turn to types. Deterministic types for the control constructs record the possible terminating choices for a term, as is standard for exception handling. Types are finite sets of choices, written as formal sums $\type I=\type{i_1+\cdots+i_n}$, and we have the following typing rules:
\[
            \infer{\term{i:i}}{} 
\qquad\quad	\infer{\term{M;i->N : I\cup J}}{\term{M:I+i} && \term{N:J}}
\qquad\quad \infer{\term{M^i : I}}{\term{M:I+i}}
\qquad\quad \infer{\term{M:I\cup J}}{\term{M:I}}
\]
As in the previous, in the probabilistic setting types become (sub-)distributions $\type{p_I}=\type{p_1i_1 * \cdots * p_ni_n}$ over a set $I$ of choices. Ground terms are then typed by the rules in Figure~\ref{fig:ground types}, where we write $\type{p_I+q_I}$ for the pointwise addition of two distributions over $I$ and $\type{p.q_I}$ for the multiplication of each probability in $\type{q_I}$ by $\type p$. A type $\type{p_I}$ implicitly assigns probability zero to any choice not in $I$, corresponding to the last rule above. Observe that the probabilistic type system is a direct generalization of the deterministic one.

\begin{figure}
\[
\begin{array}{c@{\qquad}c@{\qquad}c@{\qquad}c}
  \infer{\term{i:1i}}{}
& \infer{\term{M;i->N: p_I * (p_J+q.`r\!_J) * q.`r\!_K}}{\term{M: p_I * p_J * qi} && \term{N: `r\!_J * `r\!_K}}
& \infer{\term{M^i:/1{1-q}.p_I}}{\term{M:p_I * qi} & (q\neq 1)}
& \infer{\term{?a.a: /12\top * /12\bot}}{}
\\ \\
\vc{\begin{tikzpicture}[thick,x=18pt,y=15pt,outer sep=0pt,inner sep=1pt,->,>=stealth]
	...
	\node[draw=red,fill=red!10,rounded corners,minimum size=16pt] (M) at (m) {$\term i$};
	\draw (0,2)--(M);
\end{tikzpicture}}
&
\vc{\begin{tikzpicture}[thick,x=24pt,y=12pt,outer sep=0pt,inner sep=1pt,->,>=stealth]
	\coordinate (m) at (0,0);
	\coordinate (n) at (1,-2);
	\node[minimum size=16pt] (n') at (n) {};
	
	\node (I) at (-1  ,-4) {$\type{p_I}\vphantom)$};
	\node (J) at ( 0  ,-5.5) {$\type{p_J+q.`r\!_J}\vphantom)$};
	\node (K) at ( 1  ,-4) {$\type{q.`r\!_K}\vphantom)$};
	
	\draw[double] (m)--(-1,-2)--(I.north);
	\draw[double] (m)--( 0,-4) (n)--( 0,-4)--(J.north);
	\draw[double] (n)--(K.north);

	\draw (0,2)--(m);
	\draw (m)--node[pos=0.6,anchor=south west]{$\type{qi}$} (n'.north);
	
	\node[draw=red,fill=red!10,rounded corners,minimum size=16pt] (M) at (m) {$\term M$};
	\node[draw=red,fill=red!10,rounded corners,minimum size=16pt] (N) at (n) {$\term N$};
\end{tikzpicture}}
&
\vc{\begin{tikzpicture}[thick,x=18pt,y=15pt,outer sep=0pt,inner sep=1pt]
	\coordinate (m) at (0,0);
	
	\node[outer sep=2pt] (I) at (-0.5,-2) {$\type{/1{1-q}.p_I}$};
	
	\draw[double,->,>=stealth] (m)--(I.north) ;
	
	\coordinate (a) at (0.85,-1.7);% \node[gray,circle,draw,inner sep=0pt,minimum size=2pt] at (a) {};
	\coordinate (b) at (1.25,-1.6);% \node[gray,circle,draw,inner sep=0pt,minimum size=2pt] at (b) {};
	
    \draw[rounded corners=5pt] (m) 
       -- (0.5,-1) .. controls (a) and (b) .. (1.25,-.75) 
       --node[pos=0.3,anchor=west]{\,$\type{qi}$} (1.25,1.5) -- (0,1.5) -- (0,1) -- (m);
...
\vc{\begin{tikzpicture}[thick,x=18pt,y=15pt,outer sep=0pt,inner sep=1pt,->,>=stealth]
	...
	\node[outer sep=2pt] (i) at (-0.8,-2) {$\type{/12\top}$};
	\node[outer sep=2pt] (j) at ( 0.8,-2) {$\type{/12\bot}$};
	
	\draw (m)--(i.north) ;
	\draw (m)--(j.north) ;
	
	\node[draw=red,fill=red!10,rounded corners,minimum height=18pt,minimum width=30pt] (M) at (m) {};
	\node at (m) {$\term{?a.a}$};
	\draw (0,2)--(M);	
\end{tikzpicture}}
\end{array}
\]
\caption{Typing rules for ground terms, with illustrating diagrams. The double arrows represent multiple branches. In the second rule, the types for $\term M$ and $\term N$ divide into overlapping parts $\type{p_J}$ and $\type{`r\!_J}$, and non-overlapping parts $\type{p_I}$ and $\type{`r\!_K}$, where $I$, $J$, and $K$ are pairwise disjoint sets of choices.}
\label{fig:ground types}
\end{figure}

In Section~\ref{sec:ground terms} we will return to ground terms, and prove a \emph{Kleene theorem} for them: ground terms are equivalent to finite Markov chains, given as probabilistic automata, where states correspond to choices. Then types compute the return distribution of an encoded Markov chain, while type inference is decidable, and typed terms are positive AST.

%We contend that our ground terms provide a simple and natural account of probabilistic computation: because they admit a system of simple types that is a direct generalization of deterministic branching and iteration; because of their equivalence with finite Markov chains; and because they combine seamlessly with the $\lambda$-calculus to give an account of higher-order probabilistic computation, as we will explore next.

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\subsubsection*{The Probabilistic FMC}

The FMC combines the control constructs with the $\lambda$-calculus via the Krivine abstract machine (KAM)~\cite{Krivine-2007}, which evaluates terms in the context of an argument stack: application $M\,N$ pushes $N$ and continues as $M$; abstraction $\lambda x.M$ pops (say) $N$ and continues as $\{N/x\}M$. This gives a canonical interpretation to the (call--by--name) $\lambda$-calculus in terms of \emph{stateful} and \emph{sequential} computation, where the stack is the state and computation proceeds sequentially via \emph{pop-} and \emph{push-}actions. Together with the control constructs, the FMC then features branching and iterating computation that inputs and outputs a stack.

Emphasizing this perspective, the FMC uses $\term{[N].M}$ for application $M\,N$ and $\term{<x>.M}$ for abstraction $\lambda x.M$. With the control constructs and generator as above, the \emph{probabilistic FMC} is as follows.
\[
...
\begin{example}
\label{ex:ho-term}
Consider the following example term.
\[
	\term{<x>.x~;~\heads -> <y>.* ~;~ \tails -> <y>.[y].[y].*}
\]
It takes a (fair or unfair) coin from the stack as $\term x$, such as our $\frac13$ probability running example term, and tosses it. On heads, it deletes the top element on the stack; on tails, it duplicates it. Both branches return with the default choice $\term *$ (``skip'').
\end{example}

\begin{remark}
The Functional Machine Calculus allows not just one, but multiple argument stacks. These are named by \emph{locations} $a,b,c,\dots$, and \emph{push} and \emph{pop} are parameterized as $\term{[N]a.M}$ and $\term{a<x>.M}$ to operate on a given stack $a$. Stacks then model various state-like effects: mutable store (as a stack of depth one), input/output (as dedicated input-only and ouput-only stacks), and probabilistic choice (as a probabilistically-generated input stack $\rnd$). The probabilistic FMC here is thus a proper fragment with two stacks: one for the $\lambda$-calculus, and a probabilistic one. In the full FMC, the generator construct $\term{?a.M}$ would be written $\term{\rnd<x>.M}$, popping from the probabilistic stack $\rnd$ and using a regular variable $x$ (since $a$ would be reserved for locations). The same technique, sampling from a random stream on the Krivine machine, is used by Ehrhard~\cite{Ehrhard-2019}.
\end{remark}

\begin{remark}
The notion of sequencing in the FMC is fundamentally the same as that of Moggi's computational metalanguage~\cite{Moggi-1991} and Levy's call--by--push--value~\cite{Levy-2003}: 
\[
	M\bind N ~=~ \term{M;N} %\quad=\quad \term{M;*->N}
\]
The difference is that the latter return a single value, while the FMC returns the whole stack: $\mathsf{return}~M~=~\term{[M].*}$. This notion of sequencing further allows the encoding of call--by--value behaviour, both in a deterministic and probabilistic setting; see~\cite{Heijltjes-2023}, respectively~\cite{Heijltjes-Majury-2025}.
\end{remark}

For the full calculus, the distribution types of ground terms are extended with types for the input stack and for the return stack of each branch. Types (or \emph{computation types}) $\type{t}$ and stack types $\type{!t}$ are given below; the latter are vectors giving a type for each term, and the former an implication from an input stack type $\type{!s}$ to a formal sum of triples $\type{!t.pi}$ of an output stack type $\type{!t}$, a probability $p$, and a choice $i$, where the restriction to $\type{pi}$ forms a (sub-)distribution.
\[
\begin{array}{l@{\qquad}r@{}l}
        \text{\emph{Types:}}	    & \type{s,t}~\coloneqq~{} & \type{!s ~=>~ !t_1.p_1i_1 * \cdots * !t_n.p_ni_n}
\\[5pt]	\text{\emph{Stack types:}}  & \type{!t}~\coloneqq~{}  & \type{t_1..t_n}
\end{array}
\qquad
\vc{\begin{tikzpicture}[thick,x=24pt,y=15pt,outer sep=0pt,inner sep=1pt,->,>=stealth]

	\coordinate (m) at (0,0);
	
	\node[outer sep=1pt] (r) at ( 0, 2) {$\type{!s}$};
	\node[outer sep=1pt] (i) at (-1,-2) {$\type{!t_1.p_1i_1}$};
	\node[outer sep=1pt] (j) at ( 1,-2) {$\type{!t_n.p_ni_n}$};
	\node at (0,-1) {$\dots$};
	
	\draw (m)--(i.north) ;
	\draw (m)--(j.north) ;
	
	\node[draw=red,fill=red!10,rounded corners,minimum size=18pt] (M) at (m) {$\term{M}$};
	\draw (r)--(M);
\end{tikzpicture}}
\]
Since stacks are last-in-first-out, identity terms are of the form $\term{<x>.<y>.[y].[x].*}$, reversing the order of variables (and returning the default choice $\star$). Type vectors follow the order of terms, implicitly reversing on the left of an implication, so that identity terms have type $\type{!t => !t.1\star}$; for the given example, $\term{<x>.<y>.[y].[x].* : s\,t => t\,s.1\star}$. Types for ground terms as distributions abbreviate computation types with empty type vectors, written $\type{\e}$.
\[
	\type{p_1i_1*\cdots*p_ni_n}\quad=\quad \type{\e~=>~\e.p_1i_1*\cdots*\e.p_ni_n}
\]

\begin{example}
In the sequential setting, with a single branch $\star$, composition of two terms $\term{M:!r=>!s.p\star}$ and $\term{N:!s=>!t.q\star}$ multiplies their probabilities, $\term{M;N : !r => !t.(pq)\star}$.
\end{example}

\begin{example}
The term of Example~\ref{ex:ho-term} cannot be typed because it features two branches with the same choice $\term*$ but different output stacks (one empty, the other holding two copies of the input $\term y$). Adapting it to use distinct choices $i$ and $j$ instead, and choosing the type for the first input to match our running example, we get the type below (for any type $\type t$ for $\term y$). 
\[
	\term{<x>.x~;~\heads -> <y>.i ~;~ \tails -> <y>.[y].[y].j ~:~ (\e=>\e./23\heads*\e./13\tails)\,t~=>~\e./23i~*~\,t\,t./13j }
\]
The term expects two inputs: the unfair coin and some argument $\type t$ for the abstractions $\term{<y>}$ of each branch (note that since both branches pop from the same input stack, both variables $\term y$ get the same type). It then produces two possible outputs: with probability $\frac 23$ the branch $i$ with the empty stack, and with probability $\frac 13$ the branch $j$ with two copies of $\term y$ on the stack.
\end{example}

As the example demonstrates, the type system is quite restrictive: probabilities are fixed, even on inputs. However, the potential of lifting these restrictions is clear, for example by introducing polymorphism (variables) for probabilities, or generalising to dependent types. The purpose of the present paper is not just to establish the basic system of simple types for AST, but to offer a clear way forward in probabilistic type systems.

\begin{remark}
\label{rem:monads}
Our distribution types $\type{!t_1.p_1i_1 * \cdots * !t_n.p_ni_n}$ are a fundamentally different construct than a \emph{probability monad} $P(A)$~\cite{Giry-1982}: the former features probability distributions at the \emph{type level}, the latter at the \emph{term level}. Since sampling is a basic construct, terms in the probabilistic FMC are inherently probabilistic in the way of a probability monad; for example, assuming a base type $\type\Z$ of integers, the following term returns $2$ or $3$ with equal probability, despite having only a trivial probability at the type level.
\[
	\term{?a.a~;~\top -> [2].*~;~\bot -> [3].*~:~\e => \Z.1\star} 
\]
\end{remark}

After exploring related work in the next section, we will define the calculus (Section~\ref{sec:PFMC}) and the type system (Section~\ref{sec:types}), and give our main result, Theorem~\ref{thm:termination}: the probability given by the type of a term is a lower bound on the probability of termination, which implies almost-sure termination if that probability is one (Section~\ref{sec:AST}). The final sections explore the expressiveness of the calculus, demonstrating that ground terms are equivalent to finite Markov chains and thus positive AST (Section~\ref{sec:ground terms}), and that the full calculus is \emph{null AST}: while the probability of termination is one, expected runtime is infinite (Section~\ref{sec:NAST}).

%In this paper, we explore this idea in the context of the FMC, introducing the \emph{probabilistic FMC} (\PFMC). This combines several key computational notions in a single cohesive package: higher-order functions through the $\lambda$-calculus, which conservatively embeds as a fragment; branching and iteration via choice labels, as introduced in the FMC~\cite{Heijltjes-2025-MFPS}; and the decomposition of probabilistic choice of the probabilistic event $\lambda$-calculus~\cite{DalLago-Guerrieri-Heijltjes-2020}. The main contribution is the typed calculus, the \PFMCT, a probabilistic variant of the typed FMC, with the main result that it guarantees almost-sure termination.

%An overview of the features of the calculus, in order of appearance in the paper, is as follows. We introduce the untyped \PFMC\ as a fragment of the FMC in Section~\ref{sec:PFMC} via its operational semantics, a simple stack machine that naturally extends the Krivine machine~\cite{Krivine-2007}. The calculus inherits confluent reduction from the FMC~\cite{Heijltjes-2025-MFPS}; while a notable feature, we omit it here as it plays no crucial r\^ole. The typed calculus \PFMCT, introduced in Section~\ref{sec:types}, conservatively extends simple types for the $\lambda$-calculus (Remark~\ref{rem:simple types embed}) and our previous probabilistic types for the PEL~\cite{Antonelli-DalLago-Pistone-2022,Heijltjes-Majury-2025} (Remark~\ref{rem:probabilistic types embed}). The main result, Theorem~\ref{thm:termination}, is that the probability given by the type of a term is a lower bound on the probability of termination, which implies almost-sure termination if that probability is one.

%In the remainder of the paper we then consider two aspects of the expressive power of the calculus. Section~\ref{sec:ground terms} considers \emph{ground terms}, the fragment of the calculus that combines branching and iteration with probabilistic choice, omitting the $\lambda$-calculus; essentially the fragment we have been using in this introduction. We observe that ground terms are equivalent to finite Markov chains (Proposition~\ref{prop:ground is markov}), and that typing, which is decidable, computes the return distribution of an encoded Markov chain (Proposition~\ref{prop:markov return}). This fragment is \emph{positive} AST (PAST): the expected runtime for a probabilistic term on the machine is finite. The full \PFMCT, being higher-order, is more expressive, and in Section~\ref{sec:NAST} we give an example term that is \emph{null} AST (NAST): while the probability of termination is one, expected runtime is infinite. The mechanism is standard: by probabilistically generating a series of terms $\term{M_n}$ as below, with exponentially decreasing probability but exponentially increasing runtime, i.e.\ $\term{M_{n+1}}$ takes double the time of $\term{M_n}$.
%\[
%	\fraction12\term{M_1}~,~\fraction14\term{M_2}~,~\fraction18\term{M_3},~\dots
%\]
...
Our type system is a conservative extension of %our previous 
the systems in \cite{Antonelli-DalLago-Pistone-2022} and \cite{Heijltjes-Majury-2025} (see Remark~\ref{rem:probabilistic types embed}). While those systems already feature higher-order functions, they do not account for either iteration or recursion.

The literature on semantics and termination properties of probabilistic computation is extensive and includes a wide range of techniques. We mention semantic domains~\cite{Dahlqvist-Kozen-2020,Kozen-1981,Vakar-Kammar-Staton-2019}, bisimulations and trace semantics~\cite{vanGlabbeek-Smolka-Steffen-1995,Silva-Sokolova-2011,Stark-Smolka-2000}, probabilistic automata~\cite{Esparza-Kucera-Mayr-2004,Etessami-Yannakakis-2009}, probabilistic coherence spaces~\cite{Crubille-Ehrhard-Pagani-Tasson-2017,Danos-Ehrhard-2011,Ehrhard-Tasson-2019}, program  logics~\cite{FerrerFioriti-Hermanns-2015,McIver-Morgan-Kaminski-Katoen-2018}, and separation logic~\cite{Barthe-Hsu-Liao-2020,Jereb-Simpson-2025}.

Type systems for probabilistic calculi are numerous and diverse as well. We distinguish two kinds. Firstly, there are deterministic or monadic types, which do not feature explicit probabilities at the type level (see Remark~\ref{rem:monads}). These tend to play a supporting role, with probabilistic properties such as AST then guarandeed by construction. Examples are~\cite{TorrezRuiz-Piedeleu-Silva-Zanasi-2024},~\cite{Staton-2017} which features coproduct-typed (but not probabilistic) iteration, and~\cite{Breuvart-DalLago-Herrou-2021} which extends G\"odel's $\mathbb{T}$ with probabilistic generators and iterators that are inherently AST.

Secondly, there are several type systems that enforce AST or PAST in probabilistic $\lambda$-calculi with recursion. These are generally highly expressive, but complex, using involved techniques such as refinement types~\cite{Avanzini-DalLago-Ghyselen-2019}, size types~\cite{DalLago-Grellois-2019}, non well founded type systems~\cite{DalLago-Padovani-2024,Das-Wang-Hoffmann-2023}, or intersection types~\cite{Beutner-Ong-2021,DalLago-Faggian-Ronchi-2021}. Such complexity is largely unavoidable: unlike the deterministic case, probabilistic termination is undecidable even for higher-order programs operating over finite datatypes~\cite{Kobayashi-DalLago-Grellois-2020}, e.g.\ in probabilistic variants of the $\lambda Y$-calculus and higher-order recursion schemes.

% \cite{Aman-Ciobanu-2019} - session types

Our approach sets itself apart in two ways. Firstly, from the FMC we inherit the combination of higher-order functions with \emph{iteration}, rather than \emph{recursion} (we comment on typing recursion in our system in Appendix~\ref{A:recursion}). Secondly, as a direct probabilistic generalisation of coproducts, our probabilistic types are a notion of \emph{simple types}. We are thus taking a ``bottom-up'' approach, consciously trading expressiveness for simplicity and extensibility: our simple types are highly restricted in the algorithms they may express, but they arise naturally from typed iteration, and we expect they will extend to more powerful type systems such as polymorphic types, intersection types, and dependent types along standard lines.

% In the present work, we deliberately choose \emph{iteration} rather than \emph{recursion} in order to keep the framework as simple as possible. Indeed, handling probabilistic recursion poses substantial technical challenges, as it typically requires probabilistic function variables at the type level, effectively leading to the use of dependent types.

%Several other works \cite{Avanzini-DalLago-Ghyselen-2019,DalLago-Grellois-2019,DalLago-Padovani-2024,Das-Wang-Hoffmann-2023} introduce type systems that guarantee almost-sure termination (AST) or positive almost-sure termination (PAST) for probabilistic $\lambda$-calculi equipped with recursion. However, these approaches generally prioritise expressive power, often at the expense of conceptual and technical simplicity. A notable exception is the work of the first author with Breuvart and Herrou on AST for extensions of G\"odel's $\mathbb{T}$~\cite{Breuvart-DalLago-Herrou-2021}. In that setting, however, one endows the underlying calculus either with recursion or with sampling from infinite distributions, rather than with iteration. In contrast, iteration is a fundamentally new ingredient of our approach and plays a central r\^ole in preserving simplicity while still capturing nontrivial probabilistic behaviour.
...
\label{sec:PFMC}
%
%The Functional Machine Calculus (FMC)~\cite{Heijltjes-2023,Heijltjes-2025-MFPS} combines the $\lambda$-calculus with imperative programming and effects in the following way. Its point of departure is the (simplified) Krivine Abstract Machine~\cite{Krivine-2007}, a call--by--name operational semantics for the $\lambda$-calculus that evaluates terms in the context of an argument stack, where \emph{application} $M\,N$ pushes $N$, and \emph{abstraction} $\lambda x.M$ pops the head of the stack and substitutes it for $x$ in $M$. To emphasise the operational intuition, \emph{push} is written $\term{[N].M}$ and \emph{pop} $\term{<x>.M}$. The calculus and the machine are then extended as follows.
%
%\begin{description}
%	\item[Effects] The machine is generalised from one to multiple argument stacks or streams, which are then used to model effects: higher-order store as stacks of depth one, input/output as pop-only and push-only streams, and probabilistic choice as a pop-only stream of boolean values. These constructions derive from the probabilistic event $\lambda$-calculus (PEL)~\cite{DalLago-Guerrieri-Heijltjes-2020}, the precursor to the FMC, which decomposes probabilistic choice $\term{M+N}$ as $\term{?a.MaN}$, with a \emph{generator} $\term{?a.M}$ that pops from a probabilistic stream of booleans and a conditional $\term{M a N}$ that chooses $\term{M}$ or $\term N$ depending on $\term a$. 
%
...
For the formal development, we first recall the terms of the probabilistic FMC from the introduction, and then give its operational semantics. The FMC further features a confluent reduction relation~\cite{Heijltjes-2025-MFPS}, which we omit here.
...
%The $\lambda$-calculus embeds by letting $\lambda x.M=\term{<x>.M}$ and $M\,N=\term{[N].M}$. The generator $\term{?a.M}$ is rendered as $\term{`{\mathsf{rnd}}<a>.M}$ in other presentations of the FMC, as a parameterised pop construct accessing a stream $\mathsf{rnd}$ that represents a random generator. The calculus is thus an orthogonal combination of the $\lambda$-calculus, in the fragment of \emph{variable}, \emph{pop}, and \emph{push}; the \emph{control} extension of the FMC, consisting of \emph{choice}, \emph{case}, and \emph{loop}; and the generator $\term{?a.M}$ of the probabilistic event $\lambda$-calculus.

%Probabilistic choice $\term{M+N}$ encodes as follows, where the \emph{case} construct associates left, $\term{M; i->N ; j->P}=\term{(M; i->N) ; j-> P}$.
%\[
%	\term{M+N} ~=~\term{?a.a ; \top->M ; \bot->N }
%\]
%In this construction, first $\term{?a}$ pops a boolean value $\top$ or $\bot$ from the probabilistic stream and substitutes it for $\term a$. The remainder then selects the relevant branch, following the semantics of the \emph{case} construct: $\term{i;i->M}=\term M$ and $\term{j;i->M}=\term j$ for $i\neq j$. We reserve the choice labels $\top$ and $\bot$ for the purpose of encoding probabilistic choice, since if $\term M$ were allowed to choose $\bot$ here, that would incorrectly trigger $\term N$ after evaluating $\term M$.
%
%\begin{example}
...
The small-step semantics is given by the Krivine machine~\cite{Krivine-2007}, which evaluates the $\lambda$-calculus via a single argument stack, with the following extensions. The generator $\term{?a.M}$ will pop from a stack of boolean values, called a \emph{trace}, which we will model as being probabilistically generated. Branching computation is implemented via a \emph{contination stack} of conditional continuations $\term{i->N}$, where $\term{M;i->N}$ pushes the continuation $\term{i->N}$, and a choice term $\term i$ discards non-matching continuations $\term{j->M}$ where $i\neq j$ until it pops a matching $\term{i->N}$. The machine terminates successfully when it reaches a choice $\term i$ and the continuation stack is empty, at which point it returns the argument stack.

% The machine itself is deterministic; the probabilistic aspect of computation is modelled by taking the trace to be probabilistically generated.

%The abstract machine is given by the Krivine machine with the following extensions. In addition to the standard argument stack, it will have a \emph{trace}, a stack of boolean values that represents the probabilistic generator, and a \emph{continuation stack} that implements the branching and iteration of the \emph{case} and \emph{loop} constructs. A term $\term M$ is evaluated in the context of these three stacks until the machine halts with a choice term $\term i$ and an empty continuation stack. The argument stack is returned as output and the choice $\term i$ is an \emph{exit status}, indicating the chosen branch of the computation. The machine itself is deterministic; the probabilistic aspect of computation is modelled by taking the trace to be probabilistically generated.
...
\]
The \emph{states} of the machine are four-tuples $(t,S,\term M,K)$. Its \emph{steps} or \emph{transitions} are given in Figure~\ref{fig:machine}. A \emph{run} is a sequence of steps, written as below. A run is \emph{successful} if it terminates in a \emph{final state} $\state\e Ti\e$ with a choice term $\term i$ and empty trace and continuation stack.
\[ 
	\step= sSMK tTNL
\]
\end{definition}
...
\noindent
Appendix~\ref{A:run} demonstrates a successful run of the machine for our running example term.
...
In addition to the machine we give a big-step operational semantics, in the form of an inductively defined evaluation relation $(\evalarrow)$ that describes the successful runs of the machine directly. Where the machine implements branching computation via the contination stack, the big-step semantics gives a more abstract view as conditional sequential composition.
...
The calculus as defined above is deterministic: it externalises the probabilistic component of evaluation to the input trace $t$. To give a probabilistic semantics, we will define an evaluation relation $\Eval SM\DT$ which takes a stack $S$ and a term $\term M$ to a certain kind of distribution $\DT$, a \emph{return family}. This is a set of traces with for each $t$ an associated pair 
$(T,i)$ of a return stack and choice such that $\eval tSMTi$ (effectively, we are only shifting where the trace $t$ is considered as a parameter, on the machine versus in the return distribution). Since the probability of a trace $t$ is $\frac12^n$ where $n$ is the length of $t$, a return family readily gives rise to a probability (sub-)distribution.

A return family collects evaluation results for distinct probabilistic choices, not further evaluation along the same trace: if it has $t$, it should not have any prefix of $t$, nor any trace extending $t$. A return family will therefore be defined over a \emph{frontier} $F$: a (possibly infinite) set of traces such that no element in $F$ is a prefix of another. The \emph{probability} $\pr F$ of a frontier $F$ is the sum probability of its traces; observe that by definition, $\pr F\leq 1$.
\[
\begin{array}{l@{}l}
...
A \emph{return family} $\DS$ then collects a return stack and choice, as a pair $(S_t,i_t)_t$, for every trace $t$ in a frontier $F$, as below. The frontier $F$ over which $\DS$ is defined is written $\floor\DS=F$, and the \emph{probability} of a return family is that of its frontier, $\pr\DS = \pr{\floor\DS}$.
\[
	\DS,\DT~\coloneqq~\{ (S_t,i_t)_t \}_{t\in F}
\]

\begin{definition}
A \emph{probabilistic evaluation} $\Eval SM\DT$ of a term $\term M$ and stack $S$ to a return family $\DT$ is given by the evaluation of $\term M$ and $\term S$ with each $t\in F$, as follows.
\[
...
\begin{example}
For our running example term we have the evaluation $\Eval \e {(\heads + (\tails + \,i))^i} \DT$ for the following return family $\DT$ over the frontier of Example~\ref{ex:frontier}.
\[
...
To work with return families we use the following notation. The \emph{sum} of two return families $\DS+\DT$ is defined if they have disjoint frontiers, $\floor\DS\cap\floor\DT=\varnothing$, and is the union of their underlying sets $\DS\cup\DT$. Likewise, \emph{inclusion} between families $\DS\subseteq\DT$ is inclusion as sets. We define the following notions below. On the left, $s\pfx F$ and $s\pfx\DS$ add $s$ as a prefix to every trace $t$ in $F$, respectively in $\DS$, and $S\,\DT$ prefixes every stack in $\DT$ with $S$. On the right, $\DS\only i$ restricts a family to the choice $i$ while $\DS\less i$ excludes the choice $i$, and $\stk\DS$ collects the stacks in $\DS$ as a set. 
\[
...
Recall from the introduction that stack types on the left of an implication $\type{=>}$ are implicitly reversed, and distribution types may implicitly expand with zero-probability summands.

We introduce the following notation to work with distribution types. The \emph{basis} $\floor{\type d}$ of a distribution type $\type d$ is the set of choices $\floor{\type d}=I$ over which it is defined. To recover the informal notation of the introduction, we define the \emph{singleton} distribution type $\type{!t.pi}=\type{\{(!t,p)_i\}}$ and the \emph{sum} $\type{g*d}=\type{g\cup d}$ of distribution types over disjoint bases $\floor{\type g}\cap\floor{\type d}$. The \emph{probability} $\pr{\type d}$ of a distribution type is the sum over its probabilities $p_i$. Below left, the notation $\type{!s\,d}$ prefixes the type vector $\type{!s}$ to each $\type{!t}$ in $\type d$, and $\type{q.d}$ multiplies each probability $p_i$ in $\type d$ by the probability $q$. Below right, $\type{d|I}$ restricts $\type d$ to the choices in $I$, and $\type{d-I}$ removes them.
\[
\begin{array}{r@{}l@{\qquad\qquad}r@{}l}
...
\begin{definition}
The \emph{typed PFMC} (\PFMCT\!) is given by the typing rules in Figure~\ref{fig:types}, where a context $\term{G}=\term{x_1:t_1,,x_n:t_n}$ is a finite function from variables to types written as a sequence, and typing judgment $\term{G |- M:t}$ assigns a term $\term M$ the type $\type t$ in the context $\term G$.
\end{definition}
...
\infer{ \term{|- ?a.a ; \top -> M ; \bot -> N : !t ~=>~ /12.g ~++~ /12.d} }{
 \infer{ \term{|- ?a.a ; \top -> M : !t ~=>~ /12.g ~*~ !t./12\bot } }{
...
The type system is a conservative extension of simple types for the $\lambda$-calculus. These embed as follows, using the default choice $\star$ (see also~\cite{Heijltjes-2023,Heijltjes-2025-MFPS,Heijltjes-Majury-2025}). For these types, the typing rules for $\mathsf{variable}$, $\mathsf{pop}$, and $\mathsf{push}$ are exactly those of the simply typed $\lambda$-calculus.
\[
...
The type system is also a conservative extension of %those of our previous work on probabilistic termination
the probabilistic type systems in~\cite{Antonelli-DalLago-Pistone-2022,Heijltjes-Majury-2025}. Using the notation of~\cite{Heijltjes-Majury-2025}, simple probabilistic types are simple types with the rationals between $0$ and $1$ as the set of base types, as below left, and embed as below right, where $\star$ is again the single available choice label.
\[
	 \typ{s,t} ~\coloneqq~ p~\mid~\typ{s\to t}
\qquad\qquad
...
\begin{remark}
For almost-sure termination, where the probabilities for a distribution type sum to one, types may be further simplified by omitting probabilities altogether, $\type{t}~\coloneqq~\type{!s => !t_1.i_1 ~*~ .. ~*~ !t_n.i_n}$. The return type then records only that each given choice has a non-zero probability. To guarantee this, it is sufficient to omit the $\mathsf{zero}$ typing rule, and restrict the implicit expansion of distribution types with zero-weighted elements. It can be observed that the remaining typing rules preserve the property that all return probabilities are non-zero. For the $\mathsf{iterate}$ rule with a premise $\term{M:!s => d * !s.pi}$, the condition $p\neq 1$, that the chance to iterate is less than one, is then automatically fulfilled if $\type d$ is non-empty.
\end{remark}
...
The intended meaning of the type system is that if $\term{|- M: \e => d}$ (i.e.\ $\term M$ is closed and requires no inputs to run on the machine) then the probability of termination for $\term M$ is at least $\pr{\type d}$. We will establish this using a reducibility argument in the style of Tait~\cite{Tait-1967}, called \emph{runnability} as it uses the machine rather than reduction. The runnability predicate directly interprets the meaning of types: a runnable term of type $\term{M:!s => d}$, given a stack $S$ of runnable terms for the types in $\type{!s}$, will probabilistically evaluate to a runnable return family $\DT$ for the type $\type{d}$, with the key property that $\pr{\DT}\geq\pr{\type d}$.
...
\noindent
To work with open terms, a \emph{substitution map} $\mu$ is a finite function from variables to terms, applied to a term as $\term{\mu M}$ which replaces every free variable $x$ in $\term M$ that is in the domain of $\mu$ with the term $\term{\mu(x)}$. For a context $\Gamma=\term{x_1:t_1},\dots,\term{x_n:t_n}$ the set $\SRUN{G}$ is a set of substitution maps $\mu$ that assigns each $x_i$ a runnable term $\term M\in\RUN{t_i}$, as follows.
\[
...
\]
Since ground terms are closed, the typing rules may further omit contexts. Thus restricted, we recover the typing rules from the introduction (with new notation).
\[
       \infer{\term{i: 1i}}{}
...
We will define Markov chains as probabilistic automata, as follows. We write $\DD X$ for the set of probability distributions over a set $X$, and write a distribution over $X=\{x_1,x_2,\dots\}$ as $\{p_1x_1,p_2x_2,\dots\}$.

\begin{definition}
A \emph{Markov chain} $C=(S,s,F,\delta)$ is given by a set of \emph{states} $S$, a \emph{starting state} $s\in S$, a set of \emph{final states} $F\subseteq S$, and a stochastic \emph{transition} function $\delta:S\less F\to\DD{S}$ from non-final states to states.
%A \emph{Markov chain}
...
\begin{example}
For our running example term $\term M$ the translation is as follows.
\[
\begin{array}{r@{~}l}
          \term{M}~= & \term{(?a.a ; \top->\heads ; \bot->(?b.b ; \top->\tails ; \bot -> i))^i}
\\[8pt] \markov{M}~= & (~\{a,b,r,s,u,\heads,\tails\}~,~r~,~\{\heads,\tails\}~,~
\\[3pt]              & ~~a\mapsto\D{/12r,/12b}~~r\mapsto\D{1\heads}
\\[3pt]              & ~~b\mapsto\D{/12s,/12u}~~s\mapsto\D{1\tails}
\\[3pt]              & ~~u\mapsto\D{1r}~)
\end{array}
\qquad\qquad
	\vc{
...
%\[
%\begin{array}{rl}
%\\[5pt] \markov{M}~=~&(~\{r,s,u,v,w,\heads,\tails\}~,~r~,~\{\heads,\tails\}~,~
%\\                   &~r\mapsto\D{/12u,/12s}
%\\                   &~s\mapsto\D{/12v,/12w}
%\\                   &~u\mapsto\D{1\heads}
%\\                   &~v\mapsto\D{1\tails}
...
%\]
%\[
%\markov{M}~=~(\{r,s,u,v,w,\heads,\tails\},~r,~r\mapsto\D{/12u,/12s}~s\mapsto\D{/12v,/12w}~u\mapsto\D{1\heads}~v\mapsto\D{1\tails}~w\mapsto\D{1r}~)
%\]
%\[
%\begin{array}{r@{}l}
...
\begin{example}
\label{ex:markov}
Consider the following Markov chain $C$, where each arrow has probability $\frac13$, and its typed interpretation $\sem C$, writing $\term{M+N+P}$ for the term $\term{//13M+//13N+//13P}$. A fully worked version is in Appendix~\ref{A:markov}.
\[
\vcenter{\hbox{
\begin{tikzpicture}[inner sep=2pt,outer sep=0pt,thick]
	\node[circle,draw] (a) at ( 90:0.6) {$a$} ;
	\node[circle,draw] (b) at (210:0.6) {$b$} ;
	\node[circle,draw] (c) at (330:0.6) {$c$} ;
	\node (i) at ( 90:1.4) {$i$} ;
	\node (j) at (210:1.4) {$j$} ;
	\node (k) at (330:1.4) {$k$} ;
	\begin{scope}[->,>=stealth]
...
\qquad
\begin{array}{@{}r@{}l}
	    C ~=~{}& (~\{a,b,c,i,j,k\},~a,~\{i,j,k\},~
\\[3pt]	       & ~~ a \mapsto \D{/13b,/13c,/13i}~b \mapsto \D{/13a,/13c,/13j}~c \mapsto \D{/13a,/13b,/13k}~)
\\[10pt]
   \sem C ~=~{}& \term{((a+(a+b+k)^c+j)^b + (a+(a+c+j)^b+k)^c+i)^a}: \type{/12i * /14j * /14k}
\end{array}
\]
\end{example}
...
Further exploring the expressive power of the calculus, in this section we demonstrate that typed terms in general are \emph{null AST} (NAST), i.e.\ AST, but not PAST. We will give an example term with infinite expected running time, despite terminating with probability one. Our example relies crucially on higher-order functions.
%
%We will use the default choice $\star$ as imperative \emph{skip}, and its case as sequential composition, $\term{M;N}=\term{M;\star ->N}$. 
Consider the following term, for a given $\term M$.
\[
...
We have demonstrated a type system for almost-sure termination in a probabilistic $\lambda$-calculus with iteration, the \PFMCT. 
%We have demonstrated a probabilistic interpretation of the branching and iteration in the FMC to give a type system for almost-sure termination, the \PFMCT. 
As a direct probabilistic generalization of the FMC and typed iteration, we believe this to be a natural account of AST. %almost-sure termination for the following reasons.

%The central idea, with which we started the introduction, is to take the familiar typed iterator $\mathsf{iter}\,f$ and make it probabilistic. Immediately, this gives an account of typed almost-sure termination. We explored the consequences of this idea within the \emph{control} extension of the FMC, consisting of the \emph{choice}, \emph{case}, and \emph{loop} constructs. In itself, this forms a minimal basis for imperative control flow, modelling conditionals, constants, case switches, exception handling, and iteration, all on simple abstract machine with a single continuation stack, both typed and untyped---unlike the iterator $\mathsf{iter}\,f$, which must be typed. Combined with our probabilistic generator, the control constructs form the \emph{ground terms} of the \PFMC, which we showed to be equivalent to finite Markov chains, where typing computes the return distribution of the chain.
...
In future work, we intend to further analyse the expressive power of the calculus and fragments of it. At present, being an extension of simple types, it is clear that the expressive power of the typed calculus for real algorithms is limited. Addressing this, we expect that the type system will generalize with polymorphism, dependent types, and intersection types along standard lines. %This is not idle speculation: the principle was already demonstrated for intersection types for higher-order store in the FMC~\cite{Heijltjes-2025-FSCD}. 
A dependently typed generalisation would be akin to Hoare logic, and might facilitate reasoning about probabilistic termination in theorem provers and give natural ways to include termination evidence in typed imperative programming languages.
...
\section{On function composition and recursion}
\label{A:recursion}

As the introduction set out, typed almost-sure termination arises naturally by replacing the sum in the standard typed iterator with a probabilistic sum. The probabilistic FMC integrates iteration into the $\lambda$-calculus, giving AST in a higher-order setting. The $\lambda$-calculus is however more commonly associated with \emph{recursion} than iteration. A pertinent question is then whether typed AST is also possible for recursion. Here, we consider the obstacles to typing probabilistic termination in a probabilistic $\lambda$-calculus without branching and iteration. We will use the type system for the probabilistic event $\lambda$-calculus, given by arrow types over probabilities $p\in\mathbb Q\cap[0,1]$.
\[
	\type{s,t}~\coloneqq~\type p~\mid~\type{s\to t}
\]

First, we compare function composition,
\[
	g\circ f~=~\lambda x.\,g\,(f\,x)
\]
against sequencing $\term{f;g}$. The particular problem is to type self-composition, $f\circ f$ (the Church numeral two). Suppose we have $f:\type{1\to/12}$, i.e.\ $f$ returns with probability $\frac12$, so that we need $f\circ f:\type{1\to/14}$. For input $x:\type 1$ we get $f\,x:\type{/12}$, as expected, but to apply $f$ again we need to modify its type to $f:\type{/12\to/14}$. This may be approached in several ways.
\begin{itemize}
	\item
Using \emph{intersection types}: we assign $f$ a collection of multiple types $f:\{\type{1\to/12},\type{/12\to/14}\}$. This is used in~\cite{Antonelli-DalLago-Pistone-2022}, among others.
	\item
Using \emph{subtyping}: we may introduce a subtyping relation that coerces $f:\type{1\to/12}$ to $f:\type{/12\to/14}$ by multiplying all probabilities by a fixed factor, in this case $\frac12$. This is a rather coarse approach, since it ignores the actual flow of computation in a term, and only works as long as the type system does not multiply any probabilities.
	\item
Using \emph{polymorphic types}: with probabilistic type variables we may assign the type $f:\type{p\to/12p}$, which instantiates to $f:\type{1\to/12}$ and $f:\type{/12\to/14}$.
\end{itemize}

While these are all reasonable ideas, they are not as simple and natural as our current approach using sequential composition and iteration, which has the intrinsic ability to multiply the probabilities of sequential computations:
\[
	\term{f:t=>t./12\star} \qquad \text{and}\qquad \term{f;f : t => t./14\star}
\]

Turning to recursion, consider the following two recursive terms $f$ and $g$, where $1:\type 1$ is a terminating constant %(we may use $\lambda x.x:\type{1\to 1}$ instead) 
and $\Omega=(\lambda x.xx)(\lambda x.xx):\type 0$ diverges.
\[
	f~=~1 \oplus (\Omega\oplus f) \qquad\qquad g~=~g\oplus g
\]
Analogous to our running example from the introduction, $f$ should return with probability $\frac23$, and indeed we are able to assign that as its type:
\[
	f:\type{/23} ~\vdash~ 1 \oplus (\Omega\oplus f): \type{/23}
\]
However, we are also able to type $g$, which diverges, with any probability $p$:
\[
	g:\type{p} ~\vdash~ g\oplus g: \type{p}
\]
The key problem is that with recursion, termination happens through \emph{weakening}, i.e.\ discarding the recursive function call. In $f$, the terminating branch $1$ discards the recursive call, while in $g$ neither branch does, but this difference is invisible to the type system. Whether a subterm will be discarded during computation is an undecidable property typically tracked using intersection types, and not usually registered in a system of simple types.

A solution, therefore, would need to register the multiple branches of a computation, and which variables are weakened or not. For example, with \emph{polymorphism} we might have the following types,
\[
	f:\type{p} ~\vdash~ 1\oplus(\Omega\oplus f): \type{/12\oplus /14p}
\]
and solve $p=\frac 12+\frac14p$ for $p=\frac23$ in the rule for fixed points.

This is an interesting direction to explore, and we may do so in future. However, unlike our approach for iteration, it requires moving beyond simple types, and is beyond the scope of the present work.
...
In Figure~\ref{fig:run} we give a successful run of the machine for our running example term, unfolded to its primitives, and the trace $\bot\,\bot\,\top$.
\[
...
\end{array}
\]
\caption{A complete run of the machine}
\label{fig:run}
\end{figure}
...
\section{Proving the runnability lemma}
\label{A:runnability}
...
\section{Encoding Markov chains}
\label{A:markov}

We elaborate on Example~\ref{ex:markov}, encoding the following Markov chain $C$, where each arrow has probability $\frac13$.
\[
\vcenter{\hbox{
\begin{tikzpicture}[inner sep=2pt,outer sep=0pt,thick]
...
\end{array}
\]
The interpretation, in stages, is as follows, again writing $\term{M+N+P}$ for the term $\term{//13M+//13N+//13P}$.
\[
\begin{aligned}
