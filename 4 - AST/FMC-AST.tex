...
\author{Ugo Dal Lago and Willem Heijltjes}
...
\]
We observe that if we replace $B+A$ by a probabilistic sum $B \oplus_p A$ with a fixed probability $p\in (0,1]$, i.e.\ the probability of choosing $B$ is non-zero, and $f$ is itself terminating, then $\mathsf{iter}\;f$ is \emph{almost surely terminating}. This opens a way to approach almost sure termination via type systems.
...
In the FMC each branch of the computation is labelled with a \emph{choice} $i,j,k,\dots$, which takes the role of a constant, an exception, a case label, or an exit status. For a term $\term{M}$ with branches $i$ and $j$, depicted below left, the term $\term{M^j}$ iterates on the branch $j$ and exits on $i$, below right. (A term may have more branches, but we restrict $\term M$ to two for simplicity of exposition.) The probability of choosing each branch is included in the typing information, indicated below in blue: $\term M$ chooses $i$ with probability $p$ and $j$ with probability $q$. These must sum to at most one, as usual, but not exacly one: $\term M$ may also diverge with probability $1-p-q$. We may render the return type of $\term M$ as $\type{p_i+q_j}$, retaining the branch labels and their probabilities (the full exposition will include further typing information).
\[
\looppic1 \qquad \looppic0
\]
The probability $\frac p{1-q}$ of choosing branch $i$ in $\term{M^j}$ may be computed in two ways. The first gives the probability of choosing $i$ at each iteration of the loop as the following geometric series, given by the recurrence $x = p + qx$, which has the expected probability as its limit.
\[
p + pq + pq^2 + pq^3 + \cdots ~=~ \frac p{1-q}
\]
For the second we observe that the typing rule for $\mathsf{iter}$ simply removes the iterated branch $A$ from the sum $B+A$ and apply this to the probabilistic setting. For $\term M$ we have the probabilities $d$, $p$, and $q$ where $d=1-p-q$ is the probability of divergence. Then for $\term{M^j}$ we remove $q$ and renormalize $d$ and $p$ to sum to one. Since $d+p=1-q$ these are given directly by $\frac d{1-q}$ and $\frac p{1-q}$.

\begin{example}
For a concrete example, if $p=\frac12$ and $q=\frac13$ then $\term M$ diverges, chooses $i$, or chooses $j$ with the ratio $\frac16:\frac12:\frac13$ or $1:3:2$. Removing $q$ leaves the ratio $1:3$ or the probabilities $\frac d{1-q}=\frac14$ and $\frac p{1-q}=\frac 34$ respectively.
\end{example}

Simplified, the typing rule for iteration $\term{M^j}$ looks as follows, where $p_I$ is a family of probabilities in a set $I=\{i_1,i_2,\dots,i_n\}$ of choice labels, multiplied with the probability $\frac 1{1-q}$ pointwise in the rule below. The double arrow is the function type and $\varepsilon$ represents the lack of an input value.
\[
\begin{array}{@{}r@{}l@{}}
	\term{M}   &{}: \type{\e => p_I + q_j}
\\ \hline \\[-10pt]
	\term{M^j} &{}: \type{\e => /1{1-q}p_I}
\end{array}	
\]

\begin{example}
Our calculus encodes a fair probabilistic choice, which we write as $\term{M+N}$. The following term then computes an unfair coin toss with probability $\frac 23$ of tails and $\frac 13$ of heads, given by the booleans $\term\bot$ and $\term\top$ as choice labels.
\[
	\term{(\bot + (\top + i))^i}
\]
The types of the iterated body and the term itself are as follows.
\[
\term{\bot + (\top + i) : \e => (/12)_\bot + (/14)_\top + (/14)_i}
\qquad
\term{(\bot + (\top + i))^i : \e => (/23)_\bot + (/13)_\top}
\]
\end{example}

The typed calculus, the probabilistically typed FMC (FMC$^\oplus$), achieves the following. It guarantees termination with the probability given by the sum of all $p$ in $p_I$, which is almost sure termination if the sum is one, in the presence of higher-order probabilistic functions. In a first-order setting it guarantees \emph{positive} almost sure termination (expected evaluation time is finite), where still it may capture all Markov chains and directly compute their expected output. In the higher-order setting, which gives access to Church numerals, there are examples of \emph{non-positive} or \emph{null} AST.
...
\[
\begin{aligned}
	    \term{(a+b+k)^c}         &: \type{\e => \e./13a + \e./13b + \e./13k}
\\[5pt]	\term{ a+(a+b+k)^c+j}~~~ &: \type{\e => \e./49a + \e./19b + \e./19k + \e./13j}
\\[5pt] \term{(a+(a+b+k)^c+j)^b} &: \type{\e => \e./12a + \e./18k + \e./38j}
\\[5pt] \term{(a+(a+c+j)^b+k)^c} &: \type{\e => \e./12a + \e./18j + \e./38k}
\\[5pt] \term{ (a+(a+b+k)^c+j)^b + (a+(a+c+j)^b+k)^c+i}~~~ &: \type{\e => \e./13a + \e./16k + \e./16j + \e./13i}
\\[5pt] \term{((a+(a+b+k)^c+j)^b + (a+(a+c+j)^b+k)^c+i)^a} &: \type{\e => \e./14k + \e./14j + \e./12i}
\end{aligned}
\]
...
\[
\begin{aligned}
	    \term{ b+c+i }~~~             &: \type{\e => \e./13b + \e./13c + \e./13i}
\\[5pt] \term{ b+c+i ; b -> a+c+j}~~~ &: \type{\e => \e./19a + \e./49c + \e./13i + \e./19j}
\\[5pt] \term{(a+b+k ; b -> a+c+j)^c} &: \type{\e => \e./12a + \e./18j + \e./38k}
\\[5pt] \term{ b+c+i ; b -> a+c+j ; c -> (a+b+k ; b -> a+c+j)^c}     &: \type{\e => \e./13a + \e./13i + \e./16j + \e./16k}
\\[5pt] \term{(b+c+i ; b -> a+c+j ; c -> (a+b+k ; b -> a+c+j)^c )^a} &: \type{\e => \e./12i + \e./14j + \e./14k}
\end{aligned}
\]
