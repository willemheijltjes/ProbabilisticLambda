...
The FMC$^\oplus$ is a new type system for the FMC~\cite{Heijltjes-2025-MFPS}, while the underlying calculus remains essentially the same. It is also a conservative extension of the \emph{sequential} probabilistic event $\lambda$-calulculus
...
\\ \\ \mathsf{sample}   & \step- {\1\,t} S {?a.M} K        t S {\{\1/a\}M} K
                   \quad  \step- {\0\,t} S {?a.M} K        t S {\{\0/a\}M} K
\end{array}
\]
...
Big-step evaluation: $(i\neq j)$
\[
\begin{array}{ccc}
     \infer{\eval \e Si Si}{}
   & \infer{\eval {rs}R{M;i->N} Tj }{\eval rRM Si & \eval sSN Tj}
   & \infer{\eval rR{M;i->N} Sj }{\eval rRM Sj}
\\\\ \infer{\eval sS{[N].M} Ti}{\eval s{S\,\term N}M Ti}
   & \infer{\eval {rs}R{M^i} Tj }{\eval rRM Si & \eval sS{M^i} Tj}
   & \infer{\eval rR{M^i} Sj }{\eval rRM Sj} 
\\\\ \infer{\eval t{S\,\term N}{<x>.M} Ti}{\eval tS{\{N/x\}M} Ti}
   & \infer{\eval {\bot\,t}S{?a.M} Ti}{\eval tS{\{\bot/a\}M} Ti}
   & \infer{\eval {\top\,t}S{?a.M} Ti}{\eval tS{\{\top/a\}M} Ti}
\end{array}
\]
