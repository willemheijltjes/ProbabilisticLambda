...
Concretely, our type system assigns explicit probability distributions to termination outcomes, allowing probabilistic behaviour to be tracked compositionally at the type level. Types then soundly guarantee almost-sure termination, even in the presence of higher-order functions. 

The typed calculus itself is a conservative extension of standard probabilistic lambda-calculi. The fragment of ground terms, which consists purely of the branching and iteration constructs with probabilistic sampling, is equivalent to finite Markov chains and is positively AST (expected evaluation time is finite). The full calculus is more expressive, which we demonstrate through an example term with infinite expected evaluation time.
...
the systems in \cite{Antonelli-DalLago-Pistone-2022} and \cite{Heijltjes-Majury-2025} (see Remark~\ref{rem:probabilistic types embed}). While those systems already feature higher-order functions, they do not account for either iteration or recursion. A key ingredient in this line of work is the use of probabilistic sampling on the Krivine machine~\cite{Krivine-2007}, a technique also used by Ehrhard~\cite{Ehrhard-2019}.
...
We first recall the terms of the probabilistic FMC from the introduction, and then give a small-step and big-step operational semantics, the former as an abstract machine and the latter as an inductive evaluation relation. The FMC further features a confluent reduction relation~\cite{Heijltjes-2025-MFPS}, which we omit here.
