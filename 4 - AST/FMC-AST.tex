...
\newcommand\stk[1]{\mathsf{stk}(#1)}
\newcommand\ret[2]{\stk{#1\only #2}}
...
A \emph{return family} $\DS$ gives the result of successfully evaluating a term for every trace in a given frontier $F$, collecting for each $t\in F$ the return stack $S_t$ and choice $i_t$ as a pair $(S_t,i_t)_t$ indexed with $t$. The frontier $F$ over which $\DS$ is defined is written $\floor\DS=F$.
\[
	\DR,\DS,\DT~\coloneqq~\{ (S_t,i_t)_t \}_{t\in F}
\qquad
	\DR,\DS,\DT~:~F\to\Stack\times\Choice
\]

\begin{definition}
The \emph{probabilistic evaluation} $\Eval SM\DT$ of a term $\term M$ in the context of a stack $S$ is given by a return family $\DT$ generated by the evaluation of $\term M$ and $\term S$ with each $t\in F$.
\[
...
\begin{example}
For the term $\term M=\term{(\heads + (\tails + \,i))^i}$ of the introduction we have $\Eval \e M \DT$ for the following return family over the frontier $F$ of Example~\ref{ex:frontier}.
\[
	\DT~=~\{ (\e,\heads)_t \mid t \in F~,~|t|~\text{is odd}~\} \cup \{ (\e,\tails)_t \mid t \in F~,~|t|~\text{is even}~\}
\] 
\end{example}

Observe that evaluation is closed under inclusion: if $\Eval RM\DT$ and $\DS\subseteq\DT$ then $\Eval RM\DS$. An evaluation $\Eval SM\DT$ gives a lower bound of $\pr{\floor\DT}$ for the probability of termination of a term $\term M$ in the context of a stack $S$.

\begin{definition}
The \emph{probability of termination} of a term $\term M$ in the context of a stack $S$ is the largest probability $\pr{\floor\DT}$ for any $\DT$ such that $\Eval SM\DT$.
\end{definition}
...
We extend a frontier $F$ with a prefix $s$ pointwise, $s\pfx F=\{st\mid t\in F\}$. 

\[
\begin{array}{l@{}l}
	    s\pfx\DS   &~=~\{ \,(S_t.i_t)_{st} \mid (S_t,i_t)_t \in \DS\,\}
\\[5pt] S\,\DT     &~=~\{ \,S\,T_t.i_t \mid T_t.i_t \in \DT \}	    
\\[5pt]	\DS\only i &~=~\{ \,S_t.i_t \in \DS \mid i_t=i \,\} 
\\[5pt]	\DS\less i &~=~\{ \,S_t.i_t \in \DS \mid i_t\neq i \,\}
\\[5pt] \stk\DS    &~=~\{ \,S_t \mid S_t.i_t \in \DS\,\}
