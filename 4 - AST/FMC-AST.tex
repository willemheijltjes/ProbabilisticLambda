...
\]
The first three are the \emph{control} constructs, introduced in \cite{Heijltjes-2025-MFPS}, and implement branching and iterating computation. The first (a \emph{choice} $\term i$) and the third (a \emph{loop} $\term{M^i}$) we have seen. The second is a \emph{case} $\term{M;i->N}$ which first evaluates $\term M$, and if $\term M$ terminates with $i$ then continues with $\term N$, otherwise discarding it. They reduce by the following rules:
\[
...
\]
Since cases are fall-through, they may be iterated to give a standard case switch. A conditional encodes as follows, where cases associate left, $\term{M; i->N ; j->P}=\term{(M; i->N) ; j-> P}$.
\[
...
\qquad\quad \infer{\term{M:I+J}}{\term{M:I}}
\]
As in the previous, in the probabilistic setting types become (sub-)distributions $\type{p_I}=\type{p_1i_1 * \cdots * p_ni_n}$ over a set $I$ of choices. Ground terms are then typed by the rules in Figure~\ref{fig:ground types}, where we write $\type{p_I+q_I}$ for the pointwise addition of two distributions over $I$ and $\type{p.q_I}$ for the multiplication by $\type p$ of each probability in $\type{q_I}$. A type $\type{p_I}$ implicitly assigns probability zero to any choice not in $I$, corresponding to the last rule above. Observe that the probabilistic type system is a direct generalization of the deterministic one.
...
The FMC combines the control constructs with the $\lambda$-calculus via the Krivine Abstract Machine (KAM)~\cite{Krivine-2007}, which evaluates terms in the context of an argument stack: appliation $M\,N$ pushes $N$ and continues as $M$; abstraction $\lambda x.M$ pops (say) $N$ and continues as $\{N/x\}M$. This gives a canonical interpretation to the (call--by--name) $\lambda$-calculus in terms of \emph{stateful} and \emph{sequential} computation, where the stack is the state and computation proceeds sequentially via \emph{pop-} and \emph{push-}actions. The FMC then combines the $\lambda$-calculus with the control constructs to give branching and iterating computation that inputs and outputs a stack.
...
For the full calculus, the distribution types of ground terms are extended with a type for the input stack and one for each possible branch. Types (or \emph{computation types}) $\type{t}$ and stack types $\type{!t}$ are given below; the latter are vectors giving a type for each term, and the former an implication from an input stack type $\type{!s}$ to a formal sum of triples $\type{!t.pi}$ of an output stack type $\type{!t}$, a probability $p$, and a choice $i$, where the restriction to $\type{pi}$ forms a (sub-)distribution.
\[
...
%
%-----
%Of course, recursion has been tried. We work with iteration because it offers a way in where other approaches have failed. Let us quickly illustrate where a standard approach fails. We try to find a probabilistic type for Church numeral 2.
%
% \f.\x.f(fx)
%
%Our previous types are simple types over fractions:
%
%  s,t  ::=  p  |  s -> t
%
%We want to have  f : 1 -> 1/2  and apply it twice so that  f.f : 1 -> 1/4 (their composition, \x.f(f(x))). But we can't: to apply f twice we need different types, f : 1 -> 1/2 and f : 1/2 -> 1/4. We need probabilistic type variables, f : p -> 1/2 p , which can generalise to f : 1/2p -> 1/4p and compose so that f.f : p -> 1/4 p. Then, what is the general type language, and what is the most general type of 2'? The type system involves sums and products of probabilities, so we have at least polynomials. We need  f : p -> P(p) , a polynomial P over p. Then we have
%
%  \f.\x.f(fx) : (p -> P(p)) -> q -> P(P(q))
%
%where P is a variable over polynomials. But now we have replicated the term structure at the type level! We have arrived at a form of intersection types simply by assuming polymorphism! For this reason we are proceeding cautiously, remaining with simple types. While we appreciate that this might appear incremental, we believe we have taken a very significant step, and that the next step is far from obvious.
%-----
...
Our type system is a conservative extension of previous systems in \cite{Antonelli-DalLago-Pistone-2022} and \cite{Heijltjes-Majury-2025} (see Remarks~\ref{rem:simple types embed} and~\ref{rem:probabilistic types embed}). While those systems already featured higher-order functions, they did not account for either iteration or recursion. A key ingredient in this line of work is the use of probabilistic sampling on the Krivine machine~\cite{Krivine~2007}, a technique also used by Ehrhard~\cite{Ehrhard-2019}.

The literature on semantics and termination properties of probabilistic computation is extensive, and includes a wide range of techniques. We mention semantic domains~\cite{Kozen-1981,Vakar-Kammar-Staton-2019,Dahlqvist-Kozen-2020}, bisimulations and trace semantics~\cite{vanGlabbeek-Smolka-Steffen-1995,Stark-Smolka-2000,Silva-Sokolova-2011}, probabilistic coherence spaces~\cite{Danos-Ehrhard-2011,Crubille-Ehrhard-Pagani-Tasson-2017,Ehrhard-Tasson-2019}, separation logic~\cite{Barthe-Hsu-Liao-2020,Jereb-Simpson-2025}, and string diagrams~\cite{Piedeleu-TorrezRuiz-Silva-Zanasi-2025}. For probabilistic $\lambda$-calculi with recursion, various type systems for AST or PAST have been proposed~\cite{Avanzini-DalLago-Ghyselen-2019,DalLago-Grellois-2019,DalLago-Padovani-2024,Das-Wang-Hoffmann-2023}

 \cite{Beutner-Ong-2021}
