...
\subsection{\texorpdfstring{Strong normalization of $\rw_\prob$}{Strong normalization of probabilistic reduction}}
...
\subsection{\texorpdfstring{Confluence of $\rw_\prob$}{Confluence of probabilistic reduction}}
...
\subsection{\texorpdfstring{Confluence of $\rw$}{Confluence of whole reduction}}
...
\begin{proof}\hfill
	\begin{enumerate}
		...
		\begin{itemize}
			...
			Since $\trm{x \in {\fv{M}}}$ there are two cases:
			\begin{itemize}
				\item 
				So, 
				$\trm{M[N +a P/x]} = \trm{M_1[N +a P/x] +b M_2[N +a P/x]} \rw_\prob^* \trm{(M_1[N/x] +a M_1[P/x]) +b (M_2[N/x] +a M_2[P/x])} \allowbreak\rw_\prob^* \trm{M_1[N/x]M_2[N/x] +a M_1[P/x]M_2[P/x]} = \trm{M[N/x] +a M[P/x]}$.
...
\\	\trm{<!a N>}	&= \trm{!a<N>}
\end{align*}
A \emph{parallel $\beta$-step} is a $\beta$-reduction $N\rw_\beta^*\trm{<N*>}$ for some labelling $\trm{N*}$ of $N$.
\end{definition}

\begin{definition}
A \emph{labelled} $p$-reduction $\trm{N*}\rw_p^*\trm{M*}$ is one of the form
\begin{align*}
	\trm{(\x.N+aM)*P} &\rw^*_p \trm{(\x.N)*P+a(\x.M)*P}
\\	\trm{\x.N+aM} &\rw_p \trm{(\x.N)+a(\x.N)}
\\	\trm{(N+aM)P} &\rw_p \trm{NP+aMP}
\\	\trm{N(M+aP)} &\rw_p \trm{NM+aNP}
\\	\trm{!b N+aM} &\rw_p \trm{(!bN)+a(!bM)}
\end{align*}
\end{definition}
