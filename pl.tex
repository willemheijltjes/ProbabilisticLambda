...
Reduction in $\PEL$ will consist of standard $\beta$-reduction $\rw_\beta$ plus an evaluation mechanism for generators and choice operators, implementing probabilistic choice. We will present two such mechanisms: \emph{projective} reduction~$\rw_\pi$ and \emph{permutative} reduction~$\rw_\perm$. While projective reduction implements the given intuition for the generator and choice operator, we relegate it to Section~\ref{sec:projective reduction} and make permutative reduction our main evaluation mechanism, for the reason that it is more fine-grained, and thus more general. 
...
\begin{defn}
\emph{Reduction} $\rw \,=\, \rw_\beta \cup \rw_\perm$ in $\PEL$ consists of \emph{$\beta$-reduction}~$\rw_\beta$ and \emph{permutative} or \emph{$\perm$-reduction}~$\rw_\perm$, both given in Figure~\ref{fig:reduction rules}.
%\[
%\begin{array}{rcl}
...
By inspection of the rewrite rules in Figure~\ref{fig:reduction rules}, we can characterize the normal forms of $\rw_\perm$ and $\rw$ as follows.
...
\begin{remark}
	The set $\PEL$ endowed with $\rw_\perm$ is a (first-order) term rewriting system over a countably infinite set of variables (denoted by $x, y, z, \dots$) and the signature $\Sigma$ given by:
	\begin{itemize}
...
\begin{proof}
	For the first-order term rewriting system $(\PEL, \rw_\perm)$  it can be defined a well-founded recursive path ordering $>$ in the sense of \cite[p. 289]{Dershowitz82} such that $\trm{M \rw_\perm N}$ implies $\trm{M}>\trm{N}$.
\end{proof}

\subsection{Confluence of $\perm$-reduction}
...
We aim to prove that $\rw \,=\, \rw_\beta \cup \rw_\perm$ is confluent.
...
\begin{proof}\hfill
	\begin{enumerate}
		...
		\item By induction on the definition of $\trm{M \rw_\perm M'}$.
		\item By induction on $\trm{M}$.
		Cases:
		\begin{itemize}
...
Observe that $\rw_\perm$ and $\rwb_\perm$ have the same normal forms. Then since $\rwb_\perm\,\subseteq\,\rws_\perm$ we have $\rwbn_\perm\,\subseteq\,\rwn_\perm$. Conversely, let $N\rwn_\perm M$. On this reduction, let $P\rw_\perm Q$ be the first step such that $P\not\rwb_\perm Q$. Then there is an $R$ such that $P\rwb_\perm R$ and $Q\rw_\perm R$. Note that we have $N\rwbs_\perm R$. By confluence, $R\rwn_\perm M$, and by induction on the sum length of paths in $\rw_\perm$ from $R$ (smaller than from $N$) we have $R\rwbn_\perm M$, and hence $N\rwbn_\perm M$.
	%On labeled terms, $\rwb_\perm \,\subseteq\, \rws_\perm$ and thus $\rwbn_\perm \,\subseteq\, \rwn_\perm$ {\color{red}(incomplete; how about:) if $N\rw_\perm M$ but $N\not\rwb_\perm M$ then for some $P$ we have $N\rwb_\perm$ and $M\rws_\perm P$;} hence $\rwbn_\perm \,=\, \rwn_\perm$.
%	In particular, $\trm{N*} \rwbn_\perm \trm{M*}$ if and only if $\trm{N*} \rwn_\perm \trm{M*}$.
\end{proof}
...
\begin{proof}
The case of a $\perm$-step $N\rw_\perm M$ is immediate, since $N^\perm=M^\perm$ and $\rwp_\beta$ is reflexive (\Cref{rmk:reflexivity}). 
...
The reduction $\rw_\perm$ evaluates probabilistic sums purely by rewriting. Here we look at an alternative \emph{projective} notion of reduction, which conforms more closely to the intuition that $\trm{!a}$ generates a probabilistic event to determine the choice $\trm{+a}$.
...
\]
{\color{red}(NOTE: The evaluation $N^+$ should be discussed earlier, to complete our reduction $\rw_\perm$ by linking our syntactic $\trm{\+}$ to semantic probabilistic sums.)}
...
Observe that projective reduction is not a rewrite relation, as it applies only in head contexts---that is, we do not evaluate inside arguments or under other generators $\ttrm{!a}$ or choices $\ttrm{+a}$.

\begin{prop}
If $N\rw_\pi M$ then there is a $P$ such that $N\rws_\perm P$ with $P^+=M$.
\end{prop}
