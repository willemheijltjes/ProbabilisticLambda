...
\\	\trm{<!a.N>}	&= \trm{!a.<N>}
\end{align*}
A \emph{parallel $\beta$-step} $N\rwp\trm{<N*>}$ or $\trm{N*}\rwp\trm{<N*>}$ is a $\beta$-reduction $N\rws_\beta \trm{<N*>}$ for some labelling $\trm{N*}$ of $N$.
\end{definition}
...
\begin{lemma} If $\trm{N*}\rwp_\prob\trm{M*}$ then $\trm{<N*>}=_\prob\trm{<M*>}$.
\[
\begin{tikzpicture}
	...
	\draw[rwp] (m-1-1) --node[left] {$\scriptstyle\beta$} (m-2-1);
	\draw[rwp] (m-1-3) --node[right]{$\scriptstyle\beta$} (m-2-3);
