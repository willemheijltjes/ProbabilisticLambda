...
\begin{definition}
	The set $\fl{M}$ of the \emph{free labels} of a term $\trm{M}$ is defined inductively as follows:
	\begin{align*}
		\fl{x} &= \emptyset & \fl{MN} &= \fl{M} \cup \fl{N}  & \fl{\x.M} &= \fl{M} \\
		\fl{!a M} &= \fl{M} \smallsetminus \{a\} & \fl{M +a N} &= \fl{M}\cup \fl{N} \cup \{a\} 
	\end{align*}
	
	A term $\trm{M}$ is \emph{labeled-closed} if $\fl{M} = \emptyset$. 
\end{definition}

From now on, we consider only labeled-closed terms.
...
\begin{proof}
	It is possible to define a well-founded recursive path ordering $>$ in the sense of \cite[p. 289]{Dershowitz82} such that $\trm{M \rw_\prob N}$ implies $\trm{M>N}$.
\end{proof}
