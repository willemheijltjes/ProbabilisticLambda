...
The following lemmata then give the required commutation properties of $\rwb_\perm$, $\rwn_\perm$, and $\rwp_\beta$. Figure~\ref{fig:confluence diagrams} gives illustrating diagrams.
...
\vcenter{\hbox{\begin{tikzpicture}
	...
	\draw[rwp] (m-1-1) --node[left] {$\beta$} (m-2-1);
	\draw[rwp,implied] (m-1-2) --node[right]{$\beta$} (m-2-2);
	\draw[rws,implied] (m-2-2) --node[below]{$\perm$} (m-2-1);
\end{tikzpicture}}}
&	(x\notin\fv M)
\end{array}
\qedhere
...
\item
  ...
  term in $\mathit{SN}^\theta$ the maximal number of steps
  to normal form.
\end{itemize}
...
\label{lemma:cloredsum}
  Each set $SN^\theta$ is closed under the following rule:
  $$
...
\begin{lem}\label{lemma:cloredbox}
  The set $\mathit{SN}^\theta$ is closed under the following rule:
  $$
  \infer{\trm{(!a.M)}L_1\ldots L_m}{\trm{M}L_1\ldots L_m}
  $$
  whenever $a\not\in L_i$ for every $1\leq i\leq m$.
\end{lem}
\begin{proof}
...
\begin{lem}
  The set $SN^\theta$ is closed under the following rule:
  $$
...
\begin{proof}
  ...
  \item
    ...
    of \ref{equ:redortbet} strictly decreases.
  \item
  ...
  \item
    ...
    because the first component of \ref{equ:redortbet}
  ...
  \item
    ...
    and we can apply the induction hypothesis to it, because
    the first component of \ref{equ:redortbet}
    stays the same, but the second one strictly decreases.
    We can then obtain that
    $\trm{P[x/L_0])}L_1\ldots L_m$
  ...
  \end{itemize}
\end{proof}
\newcommand{\RedSet}[2]{\mathit{Red}^{#1}_{#2}}
The definition of a reducible term stays the same:
\begin{align*}
  \RedSet{\theta}{\alpha}&=\{(\Gamma,M)\mid M\in\mathit{SN}^\theta\wedge\judg{\Gamma}{M}{\alpha}\};\\
  \RedSet{\theta}{\tau\arrow\rho}&=\{(\Gamma,M)\mid(\judg{\Gamma}{M}{\tau\arrow\rho})\wedge \forall(\Gamma,\Delta,N)\in\RedSet{\theta}{A}.(\Gamma,\Delta,MN)\in\RedSet{\theta}{\rho}\}.
\end{align*}
\begin{lem}\label{lemma:redprop}
  \begin{enumerate}
  \item
    If $(\Gamma,M)\in\RedSet{\theta}{\tau}$, then $M\in\mathit{SN}^\theta$;
  \item
    If $\judg{\Gamma}{x L_1\ldots L_m}{\tau}$ and $L_1,\ldots,L_m\in\mathit{SN}^\theta$,
    then $(\Gamma,x L_1\ldots L_m)\in\RedSet{\theta}{\tau}$.
  \item
    If $(\Gamma,\trm{M[L_0/x]}L_1\ldots L_m)\in\RedSet{\theta}{\tau}$,
    $\judg{\Gamma}{L_0}{\rho}$ and $L_0\in\mathit{SN}^\theta$
    then $(\Gamma,\trm{(\x.M)}L_0\ldots L_m)\in\RedSet{\theta}{\tau}$.
  \item
    If $(\Gamma,ML_1\ldots L_m)\in\RedSet{\theta}{\tau}$
    and $(\Gamma,NL_1\ldots L_m)\in\RedSet{\theta}{\tau}$,
    then $(\Gamma,\trm{(M +a N)}L_1\ldots L_m)\in\RedSet{\theta}{\tau}$.
  \item
    If $(\Gamma,ML_1\ldots L_m)\in\RedSet{\theta}{\tau}$,
    then $(\Gamma,\trm{(!a.M)}L_1\ldots L_m)\in\RedSet{\theta}{\tau}$.
  \end{enumerate}
\end{lem}
\begin{proof}
...
\begin{proposition}
  ...
  $(\Gamma,N_j)\in\RedSet{\theta}{\tau_j}$ for every $1\leq j\leq
  n$. Then
  $(\Gamma,M[N_1/y_1,\ldots,N_n/y_n])\in\RedSet{\theta}{\rho}$.
\end{proposition}
\begin{proof}
  ...
  \item
    If $M$ is an application $LP$, then there exist
    a type $\xi$ such that
    ...
    that
    $$
    (\Gamma,\trm{L[\overline{N}/\overline{y}]})\in\RedSet{\theta}{\xi\arrow\rho}
    \qquad
    (\Gamma,\trm{P[\overline{N}/\overline{y}]})\in\RedSet{\theta}{\xi}
    $$
    By definition, we get
    $$
    (\Gamma,\trm{(LP)[\overline{N}/\overline{y}]})\in\RedSet{\theta}{\rho}
    $$
  \item
    If $M$ is an abstraction $\trm{\x.L}$, then $\rho$
    is an arrow type $\xi\arrow\mu$ and
    $\judg{y_1:\tau_1,\ldots,y_n:\tau_n,x:\xi}{L}{\mu}$.
    Now, consider any $(\Gamma\Delta,P)\in\RedSet{\theta}{\xi}$.
    Our objective is to prove with this hypothesis
    that $(\Gamma\Delta,\trm{(\x.L[\overline{N}/\overline{y}])}P)\in\RedSet{\theta}{\mu}$
    By induction hypothesis, since $(\Gamma\Delta,N_i)\in\RedSet{\theta}{\tau_i}$,
    we get that $(\Gamma\Delta,\trm{L[\overline{N}/\overline{y},P/x]})\in\RedSet{\theta}{\mu}$.
