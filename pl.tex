...
\end{proof}
Of course, there is no hope to prove $\rw$ being strongly normalizing,
...
\begin{proof}
	By Newman's lemma and strong normalization of $\rw_\perm$ (\Cref{lemma:strong-normalization}), confluence follows from local confluence. 
	The proof of local confluence consists in joining all critical pairs given by $\rw_\perm$.
	\SLV{Detalis are in the Appendix of \cite{arXiv}.}{Detalis are in the Appendix (p.~\pageref{lemmaAppendix:confluence-perm}).}
\end{proof}
...
Let $\Streams=\{0,1\}^\mathbb{N}$ be the space of streams of bits, ranged over by $r,s,t$.
...
In effect, the way the states above states are labeled is deterministic: even if a generator remains unlabeled in $M$, and becomes labeled after a ($\beta$-)step $M\rw N$, what label it receives is predetermined. The deep reason is that the stream-labeling algorithm assigns an outcome to each generator in a way that corresponds to a call-by-name strategy for probabilistic reduction.
